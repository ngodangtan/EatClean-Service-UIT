...
import { getEligibleRecipes } from '../services/rag/kbSampler.js';
import {
...
// Seasonings, aromatics, cooking fats and condiments that are never the primary
...
export async function generateMealPlan(req, res) {
  try {
    ...
    // Step 4b: Pre-check — verify KB has enough eligible recipes for the full plan duration.
    // Each recipe is used at most once (no repeats), so the eligible pool must be >= templateDays.
    const uniqueMealTypes = [...new Set(nutritionPlan.mealDistribution.map(d => d.mealType))];
    for (const mt of uniqueMealTypes) {
      const available = getEligibleRecipes({
        mealType: mt,
        diseases: allDiseaseKeys,
        cuisines: healthProfile.cuisinePreference ?? [],
        goal: nutritionPlan.goal,
        excludeIds: new Set()
      }).length;
      if (available < templateDays) {
        return res.status(400).json({
          message: `Kho công thức chỉ có ${available} công thức phù hợp cho bữa ${mt}, cần ít nhất ${templateDays} để tạo kế hoạch ${templateDays} ngày không lặp lại. Vui lòng bổ sung thêm công thức.`,
          reason: 'insufficient_recipes',
          mealType: mt,
          available,
          needed: templateDays
        });
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        // Per-mealType recipe ID tracking — each recipe used at most once (no-repeat guarantee)
        const usedRecipeIdsByMealType = {};
        for (const dist of nutritionPlan.mealDistribution) {
          usedRecipeIdsByMealType[dist.mealType] = new Set();
        }
        ...
        for (let dayIndex = 0; dayIndex < templateDays; dayIndex++) {
          ...
          for (const dist of nutritionPlan.mealDistribution) {
            dayEligiblePools[dist.mealType] = getEligibleRecipes({
              ...
              cuisines: healthProfile.cuisinePreference ?? [],
              goal: nutritionPlan.goal,
              excludeIds: usedRecipeIdsByMealType[dist.mealType]
            });
          }
          ...
          const dayTasks = nutritionPlan.mealDistribution.map(dist => () => {
            ...
            return generateMeal({
              ...
              eligibleRecipes: dayEligiblePools[dist.mealType]
            }, callBudget);
          });
          ...
          for (let mealIdx = 0; mealIdx < dayResults.length; mealIdx++) {
            ...
            if (hasDiseases) {
              ...
              for (let regenAttempt = 0; regenAttempt <= MAX_MEAL_REGEN_ATTEMPTS; regenAttempt++) {
                ...
                if (regenAttempt < MAX_MEAL_REGEN_ATTEMPTS && callBudget.remaining > 0) {
                  try {
                    // Exclude the failed recipe so LLM picks a different one
                    const regenEligible = dayEligiblePools[dist.mealType].filter(r => r.name !== mealContent.name);
                    if (!regenEligible.length) break;
            ...
            // Mark this recipe as used so it cannot be selected again on future days
            const selectedRecipe = dayEligiblePools[dist.mealType].find(r => r.name === mealContent.name);
            if (selectedRecipe) {
