...
import { sampleKBRecipe } from '../services/rag/kbSampler.js';
...
// Local LM Studio can only run one inference at a time. Sending concurrent
// requests causes memory spikes that crash the server. Keep this at 1.
  ...
  try {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      try {
        ...
        const usedMealNames = {};
        // Per-mealType ingredient frequency counter — flags overused ingredients
        const usedIngredientCounts = {};
        for (const dist of nutritionPlan.mealDistribution) {
          usedMealNames[dist.mealType] = [];
          usedIngredientCounts[dist.mealType] = {};
        }
        ...
        for (let dayIndex = 0; dayIndex < templateDays; dayIndex++) {
          ...
          // Capture per-mealType snapshots before any async work this day
          const dayAvoidNames = {};
          const dayAvoidIngredients = {};
          const dayInspirations = {};
          // Ingredients used 3+ times across all previous days are flagged as overused
          const INGREDIENT_OVERUSE_THRESHOLD = 3;
          for (const dist of nutritionPlan.mealDistribution) {
            // Deduplicate and cap at 5 most-recent names to prevent the prompt from
            // growing unboundedly and exceeding LM Studio's context window on later days.
            dayAvoidNames[dist.mealType] = [...new Set(usedMealNames[dist.mealType])].slice(-5);
            // Build overused ingredient ban list (sorted by frequency, capped at 8)
            dayAvoidIngredients[dist.mealType] = Object.entries(usedIngredientCounts[dist.mealType])
              .filter(([, count]) => count >= INGREDIENT_OVERUSE_THRESHOLD)
              .sort(([, a], [, b]) => b - a)
              .slice(0, 8)
              .map(([ing]) => ing);
            const kb = sampleKBRecipe({ mealType: dist.mealType, diseases: allDiseaseKeys, dayIndex });
            dayInspirations[dist.mealType] = kb
              ? `${kb.name} (${kb.ingredients.join(', ')})`
              : null;
          }
          ...
          const dayTasks = nutritionPlan.mealDistribution.map(dist => () => {
            ...
            return generateMeal({
              ...
              dayMealInspiration: dayInspirations[dist.mealType],
              avoidPrimaryIngredients: dayAvoidIngredients[dist.mealType]
            }, callBudget);
          });
          ...
          for (let mealIdx = 0; mealIdx < dayResults.length; mealIdx++) {
            ...
            // Count ingredient frequency so overused ingredients can be banned on future days
            for (const ing of mealContent.ingredients || []) {
              const key = ing.toLowerCase().trim();
              if (key) {
                usedIngredientCounts[dist.mealType][key] =
                  (usedIngredientCounts[dist.mealType][key] || 0) + 1;
              }
            }
        ...
        // ── Deduplication pass ────────────────────────────────────────────
        // The LLM sometimes generates the same name despite the avoidMealNames
        // instruction. Scan for exact-name collisions per mealType and regenerate
        // once. This is deterministic — no reliance on LLM compliance.
        {
          const seenByType = {}; // mealType → Map<name, firstDayIndex>
          for (let i = 0; i < validatedMeals.length; i++) {
            const meal = validatedMeals[i];
            const { dayIndex, dist } = mealMeta[i];
            const mtype = dist.mealType;
            if (!seenByType[mtype]) seenByType[mtype] = new Map();

            if (seenByType[mtype].has(meal.name) && callBudget.remaining > 0) {
              logger.warn(`Duplicate "${meal.name}" on Day ${dayIndex + 1} ${mtype} — regenerating`);
              const avoidAll = [...seenByType[mtype].keys()];
              try {
                const regen = await generateMeal({
                  mealType: dist.mealType,
                  calories: dist.calories,
                  protein: dist.protein,
                  carbs: dist.carbs,
                  ...
                  preferredIngredients,
                  dayNumber: dayIndex + 1,
                  avoidMealNames: avoidAll,
                  dayMealInspiration: (() => {
                    const kb = sampleKBRecipe({ mealType: dist.mealType, diseases: allDiseaseKeys, dayIndex });
                    return kb ? `${kb.name} (${kb.ingredients.join(', ')})` : null;
                  })(),
                  errorFeedback: `"${meal.name}" is already used on another day. You MUST create a completely different dish.`
                }, callBudget);
                // Sync usedMealNames: replace old name with regenerated name
                const oldName = meal.name;
                const nameIdx = usedMealNames[mtype].lastIndexOf(oldName);
                if (nameIdx !== -1) usedMealNames[mtype][nameIdx] = regen.name;
                // Sync ingredient counts: decrement old meal's ingredients, increment new
                for (const ing of meal.ingredients || []) {
                  const key = ing.toLowerCase().trim();
                  if (key && usedIngredientCounts[mtype][key]) {
                    usedIngredientCounts[mtype][key] = Math.max(0, usedIngredientCounts[mtype][key] - 1);
                  }
                }
                for (const ing of regen.ingredients || []) {
                  const key = ing.toLowerCase().trim();
                  if (key) usedIngredientCounts[mtype][key] = (usedIngredientCounts[mtype][key] || 0) + 1;
                }
                validatedMeals[i] = regen;
                seenByType[mtype].set(regen.name, dayIndex);
              } catch (dedupErr) {
                logger.error(`Dedup regen failed Day ${dayIndex + 1} ${mtype}:`, dedupErr.message);
                seenByType[mtype].set(`${meal.name}_d${dayIndex}`, dayIndex);
              }
            } else {
              seenByType[mtype].set(meal.name, dayIndex);
            }
          }
        }
